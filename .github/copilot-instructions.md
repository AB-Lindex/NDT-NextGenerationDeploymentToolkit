...
│   ├── DeploymentGroups.json      ← named groups: ordered steps referencing DeploymentActions.json keys
│   ├── DeploymentActions.json     ← action definitions: scripts, Reboot, AutoLogon
...
- `New-NDTPEImage` (alias `Build-NDTPEImage`) — builds the WinPE WIM, two bootable ISOs, and updates the WDS boot image. Emits **two** ISOs into `Boot\` (skip both with `-SkipISO`): `boot2026.iso` = hybrid BIOS/EFI via `MakeWinPEMedia` (Gen 1 VMs + reference-image capture; keeps the "Press any key to boot from CD" prompt) and `boot2026-uefi.iso` = UEFI-only no-prompt via `oscdimg` + `efisys_noprompt.bin` (Gen 2 VMs; boots straight into WinPE so automation is not blocked by a keypress). Both are built from the same staging tree — only the El Torito boot sector differs. Gen 2 boots the no-prompt ISO fine **with Secure Boot enabled** (no need to disable it), and the firmware boot order reverts to disk automatically after the WinPE phase.
